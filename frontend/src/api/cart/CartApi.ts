...
export const CartApi = {
    ...
    addItemToCart: async (userId: string, itemDto: CartItemAddingDto): Promise<CartDto> => {
        const response = await ApiClient.post<CartDto>(`/users/${userId}/cart/items`, itemDto);
        return response.data;
    },

    changeCartItemQuantity: async (userId: string, itemId: number, quantity: number): Promise<CartItemDto> => {
