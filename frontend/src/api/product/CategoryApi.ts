...
export const CategoryApi = {
    ...
    getCategoryTree: async (id: number): Promise<CategoryTreeDto> => {
        const response = await ApiClient.get<CategoryTreeDto>(`/categories/${id}/tree`);
        return response.data;
    },

    createCategory: async (categoryDto: CategoryCreatingDto): Promise<CategoryDto> => {
        const response = await ApiClient.post<CategoryDto>("/categories", categoryDto);
        return response.data;
    },

    updateCategory: async (id: number, categoryDto: CategoryUpdatingDto): Promise<CategoryDto> => {
        const response = await ApiClient.put<CategoryDto>(`/categories/${id}`, categoryDto);
        return response.data;
    },
