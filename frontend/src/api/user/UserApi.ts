...
export const UserApi = {
  ...
  assignRole: async (id: string, role: Role): Promise<UserDto> => {
    const response = await ApiClient.post<UserDto>(`/users/${id}/role`, {
      role,
    });
    return response.data;
  },
  ...
  unassignRole: async (id: string, role: Role): Promise<UserDto> => {
    const response = await ApiClient.delete<UserDto>(`/users/${id}/role`, {
      data: { role },
    });
    return response.data;
  },
  ...
  activateUser: async (
    id: string,
    statusDto: StatusHistoryCreationDto
  ): Promise<StatusHistoryDto> => {
    const response = await ApiClient.post<StatusHistoryDto>(
      `/users/${id}/activate`,
      statusDto
    );
    return response.data;
